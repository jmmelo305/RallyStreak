...
// ─── MATERIALS ───────────────────────────────────────────────────────────────
const mCourt   = new THREE.MeshPhongMaterial({ color: 0x2e6320, shininess: 10 });
const mGrass   = new THREE.MeshLambertMaterial({ color: 0x1a3d0f });
const mLine    = new THREE.MeshPhongMaterial({ color: 0xf0f0f0, emissive: 0x222222, shininess: 5 });
const mNet     = new THREE.MeshPhongMaterial({ color: 0xcccccc, side: THREE.DoubleSide, shininess: 30 });
const mNetPost = new THREE.MeshPhongMaterial({ color: 0xaaaaaa, shininess: 40 });
const mBall    = new THREE.MeshPhongMaterial({ color: 0xccff00, emissive: 0x334400, shininess: 100, specular: 0x66ff00 });
const mOppBody = new THREE.MeshPhongMaterial({ color: 0xff5511, shininess: 20 });
const mOppHead = new THREE.MeshPhongMaterial({ color: 0xe8b87a, shininess: 15 });
const mRacket  = new THREE.MeshPhongMaterial({ color: 0xddff00, emissive: 0x334400, shininess: 120, specular: 0xffffff });
...
function buildCourt() {
  ...
  // Court surface with texture detail
  const courtGeo = new THREE.PlaneGeometry(COURT_W, COURT_L, 20, 40);
  const court = new THREE.Mesh(courtGeo, mCourt);
  court.rotation.x = -Math.PI / 2;
  court.position.y = 0.002;
  court.receiveShadow = true;
  scene.add(court);

  // Line helpers with better visibility
  ...
  // Enhanced net posts with detail
  const postGeo = new THREE.CylinderGeometry(0.04, 0.04, NET_H + 0.12, 16);
  [-hw - 0.5, hw + 0.5].forEach(x => {
    ...
    const cap = new THREE.Mesh(capGeo, mNetPost);
    cap.position.set(x, NET_H + 0.12, 0);
    cap.castShadow = true;
    scene.add(cap);
  });

  // Enhanced net with better mesh
  const netGroup = new THREE.Group();
  scene.add(netGroup);
  
  // Net horizontal bands with transparency
  for (let i = 0; i <= 16; i++) {
    const y = (i / 16) * NET_H;
    const m = new THREE.Mesh(new THREE.PlaneGeometry(COURT_W + 1, 0.01), mNet);
    m.position.set(0, y, 0);
    netGroup.add(m);
  }
  
  // Net vertical straps with more detail
  for (let xi = -8; xi <= 8; xi++) {
    const x = xi * (COURT_W / 16);
    const m = new THREE.Mesh(new THREE.PlaneGeometry(0.008, NET_H), mNet);
    m.position.set(x, NET_H / 2, 0);
    netGroup.add(m);
  }
  
  // Add net tension cable
  const cableGeo = new THREE.CylinderGeometry(0.01, 0.01, COURT_W + 2, 8);
  const cable = new THREE.Mesh(cableGeo, new THREE.MeshPhongMaterial({ color: 0x666666 }));
  cable.rotation.z = Math.PI / 2;
  cable.position.set(0, NET_H, 0);
  scene.add(cable);
...
// Enhanced body with better geometry
const body = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.15, 1.05, 16), mOppBody);
body.position.y = 0.525;
body.castShadow = true;
oppGroup.add(body);

// Detailed head
const head = new THREE.Mesh(new THREE.SphereGeometry(0.16, 16, 16), mOppHead);
head.position.y = 1.22;
head.castShadow = true;
oppGroup.add(head);

// Enhanced arm with joints
const upperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.3, 12), mOppBody);
upperArm.position.set(0.25, 0.85, 0);
upperArm.rotation.z = -0.3;
upperArm.castShadow = true;
oppGroup.add(upperArm);

const lowerArm = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.25, 12), mOppBody);
lowerArm.position.set(0.45, 0.65, 0);
lowerArm.rotation.z = -0.6;
lowerArm.castShadow = true;
oppGroup.add(lowerArm);

// Enhanced racket with frame detail
const rktFrame = new THREE.Mesh(new THREE.TorusGeometry(0.22, 0.025, 8, 24), mRacket);
rktFrame.position.set(0.65, 0.55, 0);
rktFrame.castShadow = true;
oppGroup.add(rktFrame);

// Racket strings
const stringGroup = new THREE.Group();
rktFrame.add(stringGroup);
for (let i = -6; i <= 6; i++) {
  const string = new THREE.Mesh(new THREE.PlaneGeometry(0.44, 0.002), new THREE.MeshBasicMaterial({ color: 0xcccccc }));
  string.rotation.x = Math.PI / 2;
  string.position.set(0, i * 0.015, 0);
  stringGroup.add(string);
}
for (let i = -6; i <= 6; i++) {
  const string = new THREE.Mesh(new THREE.PlaneGeometry(0.002, 0.44), new THREE.MeshBasicMaterial({ color: 0xcccccc }));
  string.position.set(i * 0.015, 0, 0);
  stringGroup.add(string);
}

// Racket handle
const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.025, 0.3, 12), mRacket);
handle.position.set(0.65, 0.35, 0);
...
// ─── BALL ────────────────────────────────────────────────────────────────────
const ballMesh = new THREE.Mesh(new THREE.SphereGeometry(BALL_R, 24, 24), mBall);
ballMesh.visible = false;
ballMesh.castShadow = true;
ballMesh.receiveShadow = true;
scene.add(ballMesh);

// Ball trail with enhanced effect
const TRAIL_LEN   = 12;
const trailMeshes = [];
for (let i = 0; i < TRAIL_LEN; i++) {
  const tm = new THREE.Mesh(
    new THREE.SphereGeometry(BALL_R * (1 - i / TRAIL_LEN) * 0.6, 8, 8),
    new THREE.MeshBasicMaterial({ 
      color: 0xccff00, 
      transparent: true, 
      opacity: (1 - i / TRAIL_LEN) * 0.4,
      emissive: 0x334400
    })
  );
  tm.visible = false;
...
const gs = {
  ...
  // Konami code state
  konamiCode: [],
  konamiPattern: ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft', 'ArrowRight', 'KeyB', 'KeyA'],
...
window.addEventListener('keydown', e => {
  ...
  // Konami code detection (only works on title screen)
  if (gs.phase === 'title') {
    gs.konamiCode.push(e.code);
    if (gs.konamiCode.length > gs.konamiPattern.length) {
      gs.konamiCode.shift();
    }
    
    // Check if pattern matches
    const matches = gs.konamiCode.every((code, index) => code === gs.konamiPattern[index]);
    if (matches && gs.konamiCode.length === gs.konamiPattern.length) {
      activateInvertColors();
      gs.konamiCode = [];
    }
  }
...
function activateInvertColors() {
  gs.invertedColors = !gs.invertedColors;
  applyColorFilter();
  showMsg('KONAMI CODE!', gs.invertedColors ? 'COLORS INVERTED!' : 'COLORS NORMAL!', '');
}

function applyColorFilter() {
  const gameWrap = document.getElementById('game-wrap');
  const konamiIndicator = document.getElementById('konami-indicator');
  
  if (gs.invertedColors) {
    gameWrap.style.filter = 'invert(1) hue-rotate(180deg)';
    konamiIndicator.classList.remove('hidden');
  } else {
    gameWrap.style.filter = 'none';
    konamiIndicator.classList.add('hidden');
  }
}
...
function returnToTitle() {
  ...
  trailPos.length = 0;
  // Reset Konami code when returning to title
  gs.konamiCode = [];
  ...
  // Apply current color filter state
  applyColorFilter();
}
...
function spawnParticles(x, y, z, hit) {
  const particleCount = hit ? 20 : 16;
  const baseColor = hit ? 0xccff00 : 0xff4444;
...
function updateParticles(dt) {
  gs.particles = gs.particles.filter(p => {
    ...
    const currentSize = p.size * s;
    p.mesh.scale.setScalar(currentSize / 0.05);
    p.mesh.material.opacity = s * 0.8;
    return true;
  });
}
